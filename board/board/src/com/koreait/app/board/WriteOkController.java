...
//		request 객체, 업로드 할 경로, 파일의 크키, 인코딩 방식, 이름변경정책
		...
		boardVO.setMemberNumber((Integer)req.getSession().getAttribute("memberNumber")); //session은 null아님 타입이 다를 때
		boardVO.setBoardTitle(multipartRequest.getParameter("boardTitle")); //타입이 다를 때 null
		boardVO.setBoardContent(multipartRequest.getParameter("boardContent")); //타입이 다를 때 null
